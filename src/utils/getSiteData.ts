...
export const getSiteData = async () => {
	...
	try {
		...
		// API请求
		const response: ResponseData = await getMonitorsData({
			api_key: apikey,
			format: 'json',
			logs: 1,
			log_types: '1-2',
...
const updateSiteStatus = (data: ProcessedData[]) => {
	...
	try {
		...
		// 更新全局状态
		changeSiteState(isAllOk ? 'normal' : hasAnyOk ? 'error' : 'allError');
...
const getMonitorsData = async (postdata: RequsetPayload) => {
